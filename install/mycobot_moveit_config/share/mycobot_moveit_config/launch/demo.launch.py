...
def generate_launch_description():
    ...
    robot_description_semantic_config = load_file(
        "mycobot_moveit_config", "config/mycobot_280.srdf"
    )
    ...
    kinematics_yaml = load_yaml(
        "mycobot_moveit_config", "config/kinematics.yaml"
    )
    ...
    ompl_planning_yaml = load_yaml(
        "mycobot_moveit_config", "config/ompl_planning.yaml"
    )
    ompl_planning_pipeline_config["move_group"].update(ompl_planning_yaml)

    # MoveIt!2 Controllers:
    moveit_simple_controllers_yaml = load_yaml(
        "mycobot_moveit_config", "config/moveit_controllers.yaml"
    )
    moveit_controllers = {
    ...
    rviz_base = os.path.join(get_package_share_directory("mycobot_moveit_config"), "launch")
    rviz_full_config = os.path.join(rviz_base, "mycobot_moveit2.rviz")
    rviz_node_full = Node(
        package="rviz2",
        executable="rviz2",
        name="rviz2",
